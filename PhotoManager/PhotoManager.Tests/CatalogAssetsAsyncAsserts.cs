...
public static class CatalogAssetsAsyncAsserts
{
        ...
        Folder folder)
    {
        ...
        Assert.AreEqual(actualDate, asset.FileDateTime.Creation.Date); // Because files are generated by tests (ThumbnailCreationDateTime and FileModificationDateTime have the same value)
        Assert.AreEqual(expectedAsset.FileDateTime.Modification.Date, asset.FileDateTime.Modification.Date);
    }
    ...
    private static void AssertAssetFromDatabaseValidity(Asset assetFromDatabase, Asset expectedAsset, Guid folderId)
    {
        ...
        Assert.AreEqual(minDate, assetFromDatabase.FileDateTime.Creation.Date); // Not saved in Db, loaded at the runtime
        Assert.AreEqual(minDate, assetFromDatabase.FileDateTime.Modification.Date); // Not saved in Db, loaded at the runtime
    }
