...
public class SettingsWindowTests
{
    ...
    [Test]
    public async Task Constructor_SettingsViewModel_SetsMediumWindowAndDataContext()
    {
        await AvaloniaTestSetup.RunOnUiThreadAsync(() =>
        {
            ...
            try
            {
                ...
                SettingsViewModel viewModel = new(_application!);
                window = new(viewModel, NullLogger<SettingsWindow>.Instance);

                Assert.That(window.Title, Is.EqualTo("Settings"));
                Assert.That(window.Width, Is.EqualTo(780));
