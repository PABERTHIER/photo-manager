...
public class ConfigurationMockExtensionsTests
{
    ...
    public void ConfigurationMockTest()
    {
        ...
        Assert.That(configuration.GetValue<string>(UserConfigurationKeys.RECENT_TARGET_PATHS_TABLE_NAME), Is.EqualTo("RecentTargetPaths"));
        Assert.That(configuration.GetValue<string>(UserConfigurationKeys.SYNC_ASSETS_DIRECTORIES_DEFINITIONS_TABLE_NAME), Is.EqualTo("SyncAssetsDirectoriesDefinitions"));
        Assert.That(configuration.GetValue<ushort>(UserConfigurationKeys.THUMBNAILS_DICTIONARY_ENTRIES_TO_KEEP), Is.EqualTo(5));
    }
}
