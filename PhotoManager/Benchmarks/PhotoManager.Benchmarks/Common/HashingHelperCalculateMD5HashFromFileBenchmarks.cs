...
[RankColumn]
[ShortRunJob]
public class HashingHelperCalculateMD5HashFromFileBenchmarks
{
    private string _filePath = null!;
    private string _tempDirectory = null!;

    [Params(64 * 1024, 10 * 1024 * 1024)]
    public int FileSize { get; set; }

    [GlobalSetup]
    public void Setup()
    {
        _tempDirectory = Path.Combine(Path.GetTempPath(), $"{nameof(HashingHelperCalculateMD5HashFromFileBenchmarks)}");
    ...
    public string Current_ReadAllBytes()
    {
        ...
        return HashingHelper.CalculateMD5Hash(data);
    }

    [Benchmark]
    public string Optimized_HashDataStream()
    {
        return CalculateMD5HashFromFile(_filePath);
    }

    private static string CalculateMD5HashFromFile(string filePath)
    {
