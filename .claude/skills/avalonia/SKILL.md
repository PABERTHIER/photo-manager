...
argument-hint: <Avalonia UI topic or task>
---

Avalonia UI topic or task: $ARGUMENTS

Use the facts below as your authoritative reference when answering or implementing.
...
├── Services/                       — UI-layer services (single-instance, theme configuration)
└── Configuration/                  — UI settings classes (UiSettings)
```
...
Converters live in `Converters/` and follow this pattern:
...
- **DataGrid**: `Avalonia.Controls.DataGrid` for tabular data (duplicates, sync definitions)
...
- Use `Dispatcher.UIThread` instead of WPF's `Dispatcher.CurrentDispatcher`
- Use Avalonia's `IValueConverter` (not WPF's)
- AXAML files use `.axaml` extension, code-behind uses `.axaml.cs`
- Application entry point is `Program.cs` with `BuildAvaloniaApp()` pattern
- Single-instance enforcement uses cross-platform named mutex via `SingleInstanceService`
